...
config = botocore.config.Config(connect_timeout=1, read_timeout=300)
...
def start_epoch(epoch, layer, last_epoch, parameter_key):
    """
...
def lambda_handler(event, context):
    ...
    if state == 'forward':
        ...
        if epoch == last_epoch and layer == 0:
            ...
            end(parameter_key=parameter_key, epoch=epoch)
            
        elif epoch < last_epoch and layer == 0:
            ...
            start_epoch(epoch=epoch+1, layer=0, last_epoch=last_epoch, parameter_key=parameter_key)
        ...
        epoch = 0
        layer = 0
        last_epoch = epoch + parameters['batches'] - 1
        parameter_key = event.get('parameter_key')
        start_epoch(epoch=epoch, layer=layer, last_epoch=last_epoch, parameter_key=parameter_key)
    
    elif state == 'continue':
        # Start the next batch
        # Get currnt state parameters for the new bacth
        epoch = event.get('epoch')
        last_epoch = epoch + parameters['batches'] - 1
        layer = 0
        parameter_key = event.get('parameter_key')
