...
def finish(parameter_key):
    ...
    try:
        ...
        sns_message = "Errors uploading final results to S3:\n" + str(e)
        publish_sns(sns_message) 
        print(e)
        raise
...
def lambda_handler(event, context):
    ...
    if event.get('state') == 'continue':
        ...
        if epoch >= parameters['epochs'] - 1:
            ...
            payload = {}
            # Initialize the state for the next batch
            payload['state'] = 'continue'
            # Start the batch with the new epoch
            payload['epoch'] = epoch + 1
            payload['parameter_key'] = event.get('parameter_key')
            ...
            print("Payload to be sent to TrainerLambda to start new batch: \n" + dumps(payload))
